...
export type PlanPreset = {
  ...
  /** Max avoid polygons kept in-memory before sending to routing engine */
  avoid_target_max: number;
  /** Max avoid polygons sent per routing engine request (HERE hard limit: 20) */
...
export const DEFAULT_PLAN_PRESET: PlanPreset = {
  ...
  avoid_target_max: 40,      // lowered from 120: HERE cap is 20, so >40 in-memory has no benefit
  routing_max_avoids: 20,    // HERE hard limit
...
export function buildPlanBody(
  ...
  const body: any = {
    ...
    routing_max_avoids: merged.routing_max_avoids,
  ...
  if (extras.require_clean !== undefined) body.require_clean = extras.require_clean;
  if (extras.heavy_transport !== undefined) body.heavy_transport = extras.heavy_transport;
  if (Array.isArray(extras.vias) && extras.vias.length > 0) body.vias = extras.vias;
  return body;
}
