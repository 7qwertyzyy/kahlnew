...
export default function Page() {
  ...
  const buildRequestVias = (resolvedInputVias: Coords[], mapControls: MapControlPoint[]) => {
    ...
    const mapCoords = mapControls.map((point) => point.coord);
    if (resolvedInputVias.length === 0) return mapCoords;
    return orderViasWithMapControls(currentRouteCoords(), resolvedInputVias, mapCoords);
  };
      ...
      const html = `
        <div class="map-popup">
          ...
          <div class="map-popup__section">
            ...
            <div class="map-popup__row"><span>Max. Breite</span><strong>${escapeHtml(fmtNum(p.max_width_m ?? p.max_width, "m", 2))}</strong></div>
            <div class="map-popup__row"><span>Max. Höhe</span><strong>${escapeHtml(fmtNum(p.max_height_m ?? p.max_height, "m", 2))}</strong></div>
            <div class="map-popup__row"><span>Max. Gewicht</span><strong>${escapeHtml(fmtNum(p.max_weight_t ?? p.max_weight, "t", 1))}</strong></div>
            <div class="map-popup__row"><span>Max. Achslast</span><strong>${escapeHtml(fmtNum(p.max_axle_t ?? p.max_axleload_t, "t", 1))}</strong></div>
  ...
  async function refreshRoadworks() {
    ...
    try {
      ...
      setRwState({
        status: "FAILED",
        error: String((e as any)?.message ?? e ?? "Baustellendaten konnten nicht geladen werden."),
      });
  ...
  useEffect(() => {
    if (!geojson) setRouteEditMode(false);
  }, [geojson]);
      ...
      <aside className="control-rail">
        <div className="control-rail__inner">
          <section className="hero-card">
            <div className="eyebrow">Schwertransport-Planer</div>
            <p>
        ...
        {routingNotices.length > 0 && (
          ...
          >
            <div style={{ fontWeight: 700, marginBottom: 6 }}>⚠ HERE Routing-Hinweise</div>
            <ul style={{ margin: 0, paddingLeft: 18, fontSize: 13, color: "#5a4800" }}>
        ...
        {planBlocked && (
          <div
            style={{
              padding: 10,
              border: planBlocked?.meta?.status === "WARN" ? "1px solid #e6c84a" : "1px solid #f2c3c3",
              background: planBlocked?.meta?.status === "WARN" ? "#fffbea" : "#fff5f5",
              borderRadius: 10,
              marginBottom: 12,
            }}
          >
            ...
            <div style={{ fontSize: 13, color: planBlocked?.meta?.status === "WARN" ? "#5a4800" : "#7a1f1f", marginBottom: 8 }}>
            ...
            {!String(planBlocked.error ?? "").includes("Bitte die angezeigte Route vor Fahrtantritt manuell prüfen.") && (
              <div
                style={{
                  fontSize: 12,
                  lineHeight: 1.45,
                  color: planBlocked?.meta?.status === "WARN" ? "#6b5500" : "#7a1f1f",
                  background: planBlocked?.meta?.status === "WARN" ? "rgba(255,255,255,0.55)" : "rgba(255,255,255,0.6)",
                  border: `1px solid ${planBlocked?.meta?.status === "WARN" ? "#eedb98" : "#f0c9c9"}`,
                  borderRadius: 10,
                  padding: "8px 10px",
                  marginBottom: 10,
                }}
                  ...
                  {(showAllWarnings ? planBlocked.warnings : planBlocked.warnings.slice(0, 6)).map((w: any, idx: number) => (
                    <li key={idx} style={{ marginBottom: 8 }}>
                      ...
                      <div style={{ fontWeight: 600 }}>
                        ...
                        {w.title || "Restriktion"}
                        {w.restriction_kind
                          ? <span style={{ fontWeight: 400, color: "#888", marginLeft: 4 }}>({w.restriction_kind})</span>
                          : null}
                      </div>
                      {w.limits && (
                        <div style={{ fontSize: 12, color: "#555", marginTop: 2 }}>
                          {typeof w.limits.axle === "number" && (
                            <span>
                              Max. Achslast: <b style={{ color: "#c0392b" }}>{w.limits.axle.toFixed(1)} t</b>
                              {typeof w.vehicle_value === "number" && w.block_reason === "AXLE"
                                ? <span style={{ color: "#555" }}> · Fahrzeug: {w.vehicle_value.toFixed(1)} t</span>
                                : null}
                              {typeof w.severity === "number" && w.block_reason === "AXLE"
                                ? <span style={{ color: "#c0392b", marginLeft: 4 }}>(+{w.severity.toFixed(1)} t)</span>
                                : null}
                            </span>
                          )}
                          {typeof w.limits.width === "number" && (
                            <span style={{ marginLeft: typeof w.limits.axle === "number" ? 8 : 0 }}>
                              Max. Breite: <b style={{ color: "#c0392b" }}>{w.limits.width.toFixed(2)} m</b>
                              {typeof w.vehicle_value === "number" && w.block_reason === "WIDTH"
                                ? <span style={{ color: "#555" }}> · Fahrzeug: {w.vehicle_value.toFixed(2)} m</span>
                                : null}
                              {typeof w.severity === "number" && w.block_reason === "WIDTH"
                                ? <span style={{ color: "#c0392b", marginLeft: 4 }}>(+{w.severity.toFixed(1)} m)</span>
                                : null}
                            </span>
                          )}
                          {typeof w.limits.weight === "number" && (
                            <span style={{ marginLeft: typeof w.limits.width === "number" || typeof w.limits.axle === "number" ? 8 : 0 }}>
                              Max. Gewicht: <b style={{ color: "#c0392b" }}>{w.limits.weight} t</b>
                              {typeof w.vehicle_value === "number" && w.block_reason === "WEIGHT"
                                ? <span style={{ color: "#555" }}> · Fahrzeug: {w.vehicle_value} t</span>
                                : null}
                          ...
                          {typeof w.limits.height === "number" && (
                            <span style={{ marginLeft: 8 }}>
                              Max. Höhe: <b style={{ color: "#c0392b" }}>{w.limits.height.toFixed(2)} m</b>
                      ...
                      {w.block_reason && !w.limits && (
                        <div style={{ fontSize: 12, color: "#c0392b" }}>{w.block_reason}</div>
                      )}
                    </li>
                  ))}
                </ul>
        ...
        <section className="panel-card">
          <div className="card-head">
            <div>
              <div className="card-title">Live-Verkehrslage</div>
          ...
          <div style={{ marginTop: 10 }}>
            <button className="ghost-button" onClick={refreshRoadworks}>Baustellen neu laden</button>
          </div>
        </section>
        ...
        <section className="panel-card route-form-card">
          <div className="card-head">
            <div>
              <div className="card-title">Route planen</div>
          ...
          {(geojson || mapControlPoints.length > 0) && (
            <div
              style={{
                ...
                padding: 12,
                borderRadius: 16,
                border: routeEditMode ? "1px solid #bfd6ff" : "1px solid #e3e7ee",
                background: routeEditMode ? "#eef5ff" : "#f8fafc",
              }}
            >
              <div style={{ display: "flex", justifyContent: "space-between", gap: 10, alignItems: "flex-start", flexWrap: "wrap" }}>
                <div style={{ flex: "1 1 220px" }}>
                  <div className="card-subtitle" style={{ marginTop: 0, color: "#1f2937" }}>
              ...
              <div style={{ display: "flex", flexWrap: "wrap", gap: 8, marginTop: 10 }}>
                <button
                  ...
                  disabled={!geojson}
                >
                  {routeEditMode ? "Bearbeitung beenden" : "Route anpassen"}
                </button>
        ...
        {steps.length > 0 && (
          <>
            <div className="legend modern-legend">
              ...
              <span><span className="dot" style={{ background: "#2563EB" }} /> Baustellenpunkte</span>
              <span><span className="dot" style={{ background: "#E67E22" }} /> Baustellenlinien</span>
            </div>
            ...
            <div className="directions">
              <div className="head">
                ...
                <button className="ghost-button" onClick={exportTxt}>Als TXT exportieren</button>
              </div>
              <ol>
                {steps.map((s, i) => (
                  <li key={i}>
            ...
            <div className="directions">
              <div className="head"><b>Befahrene Straßen (dedupliziert)</b></div>
        ...
        <div className="map-frame">
          <div className="map-topbar">
            ...
            <div className="map-status-badges">
              ...
              <span className="status-badge status-badge--info">Blau = Baustellenpunkte</span>
              <span className="status-badge status-badge--roadwork">Orange = Baustellenlinien</span>
            </div>
          </div>
