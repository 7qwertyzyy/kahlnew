...
export async function POST(req: NextRequest) {
  ...
  if (!r.ok) {
    if (parsed?.code === "PGRST205" || parsed?.code === "42883" || parsed?.code === "42P01" || r.status === 404) {
      console.warn("[RESTRICTIONS] RPC not found — returning empty");
      return NextResponse.json({ type: "FeatureCollection", features: [], meta: {} });
    }
    return NextResponse.json(
      {
